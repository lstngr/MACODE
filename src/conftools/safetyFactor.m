...
%   q = SAFETYFACTOR(obj,npts,target) computes the local safety factor for
%   a magnetic configuration object (mConf) along a straight path extending
%   from the configuratin's center to the point described by coordinates in
%   target. npts indicates how many points are sampled. By default, the
%   most inner point is skipped to avoid numerical errors (this behavior
%   can be modified, see 'SkipFirst' below).
%
%   [q,p] = SAFETYFACTOR(obj,npts,target) does the same as above, but returns the
...
try
    ...
    nf = ind2sub(size(p),find(~isfinite(p)));
